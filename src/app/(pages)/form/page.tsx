...
export default function Form() {
  ...
  const {setName,setAge,setGender} = useUserContext()
  ...
  const handleClick = () => {
    setName(formData.name), setAge(formData.age), setGender(formData.gender)
    router.push("/test")
  };

  return (
    <>
    <form className="max-w-[55%]  pb-[4rem] mx-auto bg-black mt-[3rem]">
      ...
      <div className="relative z-0 w-full mb-[4rem] group">
        <input
          ...
          name="name"
          id="name"
          className="block py-2.5 px-0 w-full text-xl text-white bg-transparent border-0 border-b-2 appearance-none  border-gray-600 focus:border-blue-500 focus:outline-none focus:ring-0  peer"
          placeholder=""
          required
        ...
        <label
          htmlFor="name"
          className="peer-focus:font-medium absolute text-lg  text-gray-400 duration-300 transform -translate-y-6 scale-75 top-2 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto  peer-focus:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
        >
          Name
        </label>
      </div>
